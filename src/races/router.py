...
async def list_races(session: AsyncSession = Depends(get_session)):
    ...
    for race in races:
        ...
        result.append(RaceShort.model_validate({
            ...
            "organizer_likes": r["likes"],
            "organizer_dislikes": r["dislikes"],
        }))
    return result
...
async def get_race(race_id: int, session: AsyncSession = Depends(get_session)):
    ...
    return RaceOut.model_validate({
        **race.__dict__,
        "creator_email": race.creator.email if race.creator else None,
        "organizer_likes": rating["likes"],
        "organizer_dislikes": rating["dislikes"],
    })
...
async def get_participants(race_id: int, session: AsyncSession = Depends(get_session)):
    ...
    return [
        {"user_id": p.user_id, "username": p.user.email}
...
async def get_results(race_id: int, session: AsyncSession = Depends(get_session)):
    ...
    return RaceResultsOut.model_validate({
        ...
        "race_name": race.name,
        "results": [
            {"user_id": r.user_id, "username": r.user.email, "position": r.position}
            for r in results
        ],
    })
...
@router.post("/{race_id}/review", response_model=ReviewOut, status_code=status.HTTP_200_OK)
...
@router.delete("/{race_id}/review", status_code=status.HTTP_204_NO_CONTENT)
