...
class TestListNews:
    ...
    async def test_list_returns_created_news(self, client, superuser):
        await client.post("/api/news/", json=NEWS_PAYLOAD, headers=superuser["headers"])
        resp = await client.get("/api/news/")
        assert resp.status_code == 200
    ...
    async def test_list_ordered_by_created_desc(self, client, superuser):
        await client.post("/api/news/", json={**NEWS_PAYLOAD, "title": "First"}, headers=superuser["headers"])
        await client.post("/api/news/", json={**NEWS_PAYLOAD, "title": "Second"}, headers=superuser["headers"])
        resp = await client.get("/api/news/")
...
class TestGetNews:
    async def test_get_existing(self, client, superuser):
        created = await client.post("/api/news/", json=NEWS_PAYLOAD, headers=superuser["headers"])
        news_id = created.json()["id"]
        resp = await client.get(f"/api/news/{news_id}")
        assert resp.status_code == 200
...
class TestCreateNews:
    async def test_create_as_superuser(self, client, superuser):
        resp = await client.post("/api/news/", json=NEWS_PAYLOAD, headers=superuser["headers"])
    ...
    async def test_create_as_regular_user(self, client, registered_user):
        resp = await client.post("/api/news/", json=NEWS_PAYLOAD, headers=registered_user["headers"])
        assert resp.status_code == 403

    async def test_create_as_verified_user(self, client, verified_user):
        resp = await client.post("/api/news/", json=NEWS_PAYLOAD, headers=verified_user["headers"])
        assert resp.status_code == 403

    async def test_create_without_auth(self, client):
        resp = await client.post("/api/news/", json=NEWS_PAYLOAD)
        assert resp.status_code == 401

    async def test_create_missing_required_field(self, client, superuser):
        bad = {k: v for k, v in NEWS_PAYLOAD.items() if k != "title"}
        resp = await client.post("/api/news/", json=bad, headers=superuser["headers"])
    ...
    async def test_create_without_optional_fields(self, client, superuser):
        ...
        resp = await client.post("/api/news/", json=minimal, headers=superuser["headers"])
        assert resp.status_code == 201
...
class TestUpdateNews:
    ...
    async def test_update_without_auth(self, client, superuser):
        created = await client.post("/api/news/", json=NEWS_PAYLOAD, headers=superuser["headers"])
        news_id = created.json()["id"]
        resp = await client.patch(f"/api/news/{news_id}", json={"title": "Update"})
        assert resp.status_code == 401
    ...
    async def test_update_all_fields(self, client, superuser):
        ...
        resp = await client.patch(f"/api/news/{news_id}", json=update_data, headers=superuser["headers"])
        assert resp.status_code == 200
...
class TestDeleteNews:
    async def test_delete_as_superuser(self, client, superuser):
        created = await client.post("/api/news/", json=NEWS_PAYLOAD, headers=superuser["headers"])
        news_id = created.json()["id"]
        resp = await client.delete(f"/api/news/{news_id}", headers=superuser["headers"])
        assert resp.status_code == 204
        get_resp = await client.get(f"/api/news/{news_id}")
        assert get_resp.status_code == 404

    async def test_delete_as_regular_user(self, client, superuser, registered_user):
        created = await client.post("/api/news/", json=NEWS_PAYLOAD, headers=superuser["headers"])
        news_id = created.json()["id"]
        resp = await client.delete(f"/api/news/{news_id}", headers=registered_user["headers"])
        assert resp.status_code == 403

    async def test_delete_nonexistent_news(self, client, superuser):
        resp = await client.delete("/api/news/99999", headers=superuser["headers"])
        assert resp.status_code == 404

    async def test_delete_without_auth(self, client, superuser):
        created = await client.post("/api/news/", json=NEWS_PAYLOAD, headers=superuser["headers"])
        news_id = created.json()["id"]
        resp = await client.delete(f"/api/news/{news_id}")
        assert resp.status_code == 401