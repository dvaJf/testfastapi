...
class TestAuthUsersEndpoints:
    ...
    async def test_get_current_user(self, client, registered_user):
        resp = await client.get("/api/auth/users/me", headers=registered_user["headers"])
    ...
    async def test_get_user_by_id(self, client, superuser, registered_user):
        me = await client.get("/api/auth/users/me", headers=registered_user["headers"])
        user_id = me.json()["id"]

        resp = await client.get(f"/api/auth/users/{user_id}", headers=superuser["headers"])
        assert resp.status_code == 200
    ...
    async def test_get_user_by_id_as_regular_user(self, client, registered_user):
        ...
        resp = await client.get(f"/api/auth/users/{user_id}", headers=registered_user["headers"])
        assert resp.status_code in (200, 403)

    async def test_get_nonexistent_user(self, client, superuser):
        resp = await client.get("/api/auth/users/99999", headers=superuser["headers"])
    ...
    async def test_delete_user_by_id_as_superuser(self, client, superuser, registered_user):
        ...
        resp = await client.delete(f"/api/auth/users/{user_id}", headers=superuser["headers"])
