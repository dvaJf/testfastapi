...
class TestLeaderboard:
    ...
    async def test_leaderboard_orders_by_score_desc(self, client, superuser):
        ...
        await client.post("/api/auth/register", json=user1_payload)
        await client.post("/api/auth/register", json=user2_payload)

        login1 = await client.post("/api/auth/login", data={"username": "user1@example.com", "password": "Pass123!"})
        login2 = await client.post("/api/auth/login", data={"username": "user2@example.com", "password": "Pass123!"})
        ...
        race_resp = await client.post("/api/races/", json={**RACE_PAYLOAD, "maxuser": 10}, headers=superuser["headers"])
        race_id = race_resp.json()["id"]

        await client.post(f"/api/races/{race_id}/register", headers=headers1)
        await client.post(f"/api/races/{race_id}/register", headers=headers2)
        await client.patch(f"/api/races/{race_id}", json={"status": "Завершена"}, headers=superuser["headers"])
    ...
    async def test_leaderboard_races_completed_count(self, client, superuser, registered_user):
        for i in range(2):
            race_resp = await client.post("/api/races/", json={**RACE_PAYLOAD, "name": f"Race {i}"}, headers=superuser["headers"])
            race_id = race_resp.json()["id"]
            await client.post(f"/api/races/{race_id}/register", headers=registered_user["headers"])
            await client.patch(f"/api/races/{race_id}", json={"status": "Завершена"}, headers=superuser["headers"])
    ...
    async def test_leaderboard_best_position(self, client, superuser, registered_user):
        race_resp1 = await client.post("/api/races/", json={**RACE_PAYLOAD, "name": "Race 1"}, headers=superuser["headers"])
        race_id1 = race_resp1.json()["id"]
        await client.post(f"/api/races/{race_id1}/register", headers=registered_user["headers"])
        await client.patch(f"/api/races/{race_id1}", json={"status": "Завершена"}, headers=superuser["headers"])

        race_resp2 = await client.post("/api/races/", json={**RACE_PAYLOAD, "name": "Race 2"}, headers=superuser["headers"])
        race_id2 = race_resp2.json()["id"]
        await client.post(f"/api/races/{race_id2}/register", headers=registered_user["headers"])
        await client.patch(f"/api/races/{race_id2}", json={"status": "Завершена"}, headers=superuser["headers"])

        parts1 = await client.get(f"/api/races/{race_id1}/all_users")
        user_id = parts1.json()[0]["user_id"]

        await client.post(
            f"/api/races/{race_id1}/results",
            json={"results": [{"user_id": user_id, "position": 5}]},
            headers=superuser["headers"],
        )
        await client.post(
            f"/api/races/{race_id2}/results",
