...
class TestGetRace:
    async def test_get_existing(self, client, sample_race):
        resp = await client.get(f"/api/races/{sample_race['id']}")
        assert resp.status_code == 200
        assert resp.json()["id"] == sample_race["id"]
        assert resp.json()["about"] is not None

    async def test_get_nonexistent(self, client):
        resp = await client.get("/api/races/99999")
        assert resp.status_code == 404


class TestCreateRace:
    async def test_create_as_superuser(self, client, superuser):
        resp = await client.post("/api/races/", json=RACE_PAYLOAD, headers=superuser["headers"])
    ...
    async def test_create_as_unverified_user(self, client, registered_user):
        resp = await client.post("/api/races/", json=RACE_PAYLOAD, headers=registered_user["headers"])
        assert resp.status_code == 403

    async def test_create_as_verified_user(self, client, verified_user):
        resp = await client.post("/api/races/", json=RACE_PAYLOAD, headers=verified_user["headers"])
        assert resp.status_code == 201

    async def test_create_without_auth(self, client):
        resp = await client.post("/api/races/", json=RACE_PAYLOAD)
        assert resp.status_code == 401

    async def test_create_missing_required_field(self, client, superuser):
        bad = {k: v for k, v in RACE_PAYLOAD.items() if k != "name"}
        resp = await client.post("/api/races/", json=bad, headers=superuser["headers"])
        assert resp.status_code == 422


class TestUpdateRace:
    async def test_update_as_creator(self, client, verified_user):
        created = await client.post("/api/races/", json=RACE_PAYLOAD, headers=verified_user["headers"])
    ...
    async def test_update_as_non_creator(self, client, superuser, verified_user):
        created = await client.post("/api/races/", json=RACE_PAYLOAD, headers=superuser["headers"])
        race_id = created.json()["id"]
        resp = await client.patch(
            f"/api/races/{race_id}",
            json={"name": "Hack"},
...
class TestRegisterForRace:
    async def test_register_twice(self, client, sample_race, registered_user):
        await client.post(f"/api/races/{sample_race['id']}/register", headers=registered_user["headers"])
        resp = await client.post(f"/api/races/{sample_race['id']}/register", headers=registered_user["headers"])
        assert resp.status_code == 400

    async def test_register_nonexistent_race(self, client, registered_user):
        resp = await client.post("/api/races/99999/register", headers=registered_user["headers"])
        assert resp.status_code == 404
    ...
    async def test_register_when_full(self, client, superuser):
        ...
        await client.post(f"/api/races/{race_id}/register", headers=superuser["headers"])
        ...
        login = await client.post(
            "/api/auth/login", data={"username": "user2@example.com", "password": "Pass123!"}
        )
    ...
    async def test_register_when_closed(self, client, superuser, registered_user):
        ...
        race_id = race_resp.json()["id"]
        resp = await client.post(f"/api/races/{race_id}/register", headers=registered_user["headers"])
        assert resp.status_code == 400

    async def test_register_without_auth(self, client, sample_race):
        resp = await client.post(f"/api/races/{sample_race['id']}/register")
        assert resp.status_code == 401


class TestUnregisterFromRace:
    async def test_unregister_not_registered(self, client, sample_race, registered_user):
        resp = await client.delete(f"/api/races/{sample_race['id']}/unregister", headers=registered_user["headers"])
        assert resp.status_code == 400

    async def test_unregister_when_closed(self, client, superuser, registered_user):
        race_resp = await client.post("/api/races/", json=RACE_PAYLOAD, headers=superuser["headers"])
        race_id = race_resp.json()["id"]
        await client.post(f"/api/races/{race_id}/register", headers=registered_user["headers"])
        # Close the race
        await client.patch(f"/api/races/{race_id}", json={"status": "Завершена"}, headers=superuser["headers"])
        resp = await client.delete(f"/api/races/{race_id}/unregister", headers=registered_user["headers"])
        assert resp.status_code == 400

class TestParticipants:
    async def test_get_participants_empty(self, client, sample_race):
        resp = await client.get(f"/api/races/{sample_race['id']}/all_users")
        assert resp.status_code == 200
        assert resp.json() == []
    ...
    async def test_participant_count_in_race(self, client, sample_race, registered_user):
        await client.post(f"/api/races/{sample_race['id']}/register", headers=registered_user["headers"])
        race = await client.get(f"/api/races/{sample_race['id']}")
...
class TestResults:
    ...
    async def test_get_results_empty(self, client, sample_race):
        resp = await client.get(f"/api/races/{sample_race['id']}/results")
        assert resp.status_code == 200
    ...
    async def test_set_results_success(self, client, superuser, registered_user):
        ...
        parts = await client.get(f"/api/races/{race_id}/all_users")
    ...
    async def test_get_results_after_set(self, client, superuser, registered_user):
        ...
        resp = await client.get(f"/api/races/{race_id}/results")
        assert resp.status_code == 200
    ...
    async def test_set_results_duplicate_positions(self, client, superuser, registered_user):
        ...
        race_resp = await client.post(
            "/api/races/", json={**RACE_PAYLOAD, "maxuser": 10}, headers=superuser["headers"]
        )
        race_id = race_resp.json()["id"]
        await client.post(f"/api/races/{race_id}/register", headers=registered_user["headers"])
        await client.post(f"/api/races/{race_id}/register", headers=superuser["headers"])
...
class TestRaceResultsPoints:
    ...
    async def _setup_race_with_results(self, client, superuser, registered_user, position):
        race_resp = await client.post("/api/races/", json={
            "name": "Points Test",
            "race": "Test",
    ...
    async def test_first_place_points(self, client, superuser, registered_user):
        ...
        resp = await client.get("/api/auth/users/leaderboard")
    ...
    async def test_results_idempotency(self, client, superuser, registered_user):
        ...
        resp1 = await client.get("/api/auth/users/leaderboard")
        ...
        resp2 = await client.get("/api/auth/users/leaderboard")
...
class TestRaceReviewsEdgeCases:
    ...
    async def test_multiple_users_review_same_organizer(self, client, superuser):
        ...
        for i in range(3):
            ...
            login = await client.post("/api/auth/login", data={"username": payload["email"], "password": payload["password"]})
        ...
        # Register all users
        for user in users:
            await client.post(f"/api/races/{race_id}/register", headers=user["headers"])

        await client.patch(f"/api/races/{race_id}", json={"status": "Завершена"}, headers=superuser["headers"])

        # All users review positively
        for user in users:
            resp = await client.post(f"/api/races/{race_id}/review", json={"vote": 1}, headers=user["headers"])
            assert resp.status_code == 200

        # Check organizer rating
        race = await client.get(f"/api/races/{race_id}")
    ...
    async def test_review_then_unregister_impossible(self, client, superuser, registered_user):
        ...
        # Submit review
        resp = await client.post(f"/api/races/{race_id}/review", json={"vote": 1}, headers=registered_user["headers"])
        assert resp.status_code == 200
